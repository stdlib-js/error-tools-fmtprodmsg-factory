...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '0Np2V,FD', options ) );
	}
	if ( hasOwnProp( options, 'url' ) ) {
		opts.url = options.url;
		if ( !isString( opts.url ) ) {
			return new TypeError( format( '0Np2W,Gh', 'url', opts.url ) );
		}
	}
	if ( hasOwnProp( options, 'message' ) ) {
		opts.message = options.message;
		if ( !isString( opts.message ) ) {
			return new TypeError( format( '0Np2W,Gh', 'message', opts.message ) );
		}
	}
	return null;
}
